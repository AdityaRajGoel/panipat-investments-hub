...
import {
  ...
  Fuel, Pill, ShoppingCart, Landmark, Factory
} from "lucide-react";
...
// Sector Heatmap
const sectorData = [
  { name: "IT", icon: Cpu, change: "+2.4%", up: true, weight: 18 },
  { name: "Banks", icon: Landmark, change: "-0.8%", up: false, weight: 25 },
  { name: "Pharma", icon: Pill, change: "+1.6%", up: true, weight: 10 },
  { name: "Auto", icon: Factory, change: "+3.1%", up: true, weight: 12 },
  { name: "Energy", icon: Fuel, change: "-1.2%", up: false, weight: 15 },
  { name: "FMCG", icon: ShoppingCart, change: "+0.5%", up: true, weight: 8 },
  { name: "Realty", icon: Building2, change: "+4.2%", up: true, weight: 5 },
  { name: "Healthcare", icon: Heart, change: "-0.3%", up: false, weight: 7 },
];
  ...
  <Card className="border-border/50 overflow-hidden">
    <CardContent className="p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[10px] text-brand-orange font-semibold">NIFTY Sectors</span>
      </div>
...
// FII/DII Flow Widget
const fiiDiiData = [
  { label: "FII (Cash)", buy: "₹8,245 Cr", sell: "₹9,490 Cr", net: "-₹1,245 Cr", up: false },
  { label: "DII (Cash)", buy: "₹6,780 Cr", sell: "₹4,600 Cr", net: "+₹2,180 Cr", up: true },
  { label: "FII (F&O)", buy: "₹42,150 Cr", sell: "₹45,320 Cr", net: "-₹3,170 Cr", up: false },
];
        ...
        {fiiDiiData.map((item) => (
          <div key={item.label} className="bg-muted/30 rounded-lg p-3">
            ...
            <div className="mt-1.5 h-1.5 rounded-full bg-muted overflow-hidden flex">
              <div className="bg-secondary/70 rounded-l-full" style={{ width: item.up ? "60%" : "45%" }} />
    ...
    <Card className="border-border/50 overflow-hidden">
      <CardContent className="p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-muted/30 rounded-lg p-3 text-center">
            <div className="text-[10px] text-muted-foreground mb-1">Max Pain</div>
            <div className="text-2xl font-bold text-brand-orange">22,200</div>
            <div className="text-[10px] text-muted-foreground">NIFTY</div>
          </div>
          <div className="bg-muted/30 rounded-lg p-3 text-center">
            <div className="text-[10px] text-muted-foreground mb-1">OI Change CE</div>
            <div className="text-lg font-bold text-destructive">+12.4L</div>
            <div className="text-[10px] text-muted-foreground">22500 Strike</div>
          </div>
          <div className="bg-muted/30 rounded-lg p-3 text-center">
            <div className="text-[10px] text-muted-foreground mb-1">OI Change PE</div>
            <div className="text-lg font-bold text-secondary">+8.7L</div>
            <div className="text-[10px] text-muted-foreground">22000 Strike</div>
          </div>
        </div>
        <div className="mt-3 flex items-center justify-between text-[10px] text-muted-foreground">
          <span>India VIX: <b className="text-foreground">13.45</b> <span className="text-destructive">(-2.1%)</span></span>
          <span className="flex items-center gap-1 text-brand-orange font-semibold cursor-pointer hover:underline">
...
// Trending Stocks Strip
const trendingStocks = [
  { name: "TATA POWER", change: "+4.8%", up: true, reason: "Q3 results beat estimates" },
  { name: "ZOMATO", change: "+3.5%", up: true, reason: "Quick commerce growth" },
  { name: "ADANI GREEN", change: "+3.9%", up: true, reason: "New solar capacity" },
  { name: "PAYTM", change: "-3.2%", up: false, reason: "Regulatory concerns" },
  { name: "COAL INDIA", change: "-1.5%", up: false, reason: "Production miss" },
];
      ...
      <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-1">
        {trendingStocks.map((stock) => (
          ...
          >
            ...
            <div className="text-[9px] text-primary-foreground/50">{stock.reason}</div>
          </motion.div>
        ))}
      </div>
    </CardContent>
  ...
  <Card className="border-border/50 overflow-hidden">
    <CardContent className="p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[10px] text-muted-foreground">Delayed 15 min</span>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <motion.div className="text-center mb-10" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <p className="text-muted-foreground text-sm max-w-lg mx-auto">
            Professional-grade market insights, institutional flows, and derivatives analytics
          </p>
        </motion.div>
